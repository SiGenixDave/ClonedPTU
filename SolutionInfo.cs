 ...
 *                                              form is disposed of.
 * 
 *                                          3.  Modified the form used to display the flag states associated with a bitmask watch variable such that the flag state
 *                                              update is triggered by an event raised by the parent form associated with the forms used to display the live watch
 *                                              variable data and the saved watch variable data.
 * 
 ...
 *                                          5.  Modified the FormPTUDialog class to automatically assert the Pause property of any multiple-document interface (MDI)
 *                                              child forms that are open if the CommunicationInterface property is defined. Also clears the Pause property on disposal. 
 * 
 ...
 *                                          6.  Modified the Common.FormPTUDialog class. Modified the 'Shown' event handler so that polling is only suspended an any
 *                                              child forms if the dialog form is called from the main application window.
 ...
 * 
 *  09/01/10    6.3.1.7     K.McD           1.  Included the classes required to support the 'Configure/System Information' menu option. Added
 *                                              FormShowSystemConfiguration, FormConfigureDateTime and FormConfigureCarIdentifier.
 * 
 ...
 *                                              and time is updated.
 *                                          5.  Modified the form used to configure the date and time so that the date and time display is now updated using the
 *                                              DataUpdate event published by the for used to display the system information.
 * 
 ...
 *                                          2.  Bug fix SNCR001.21. Included support for the <IPollTarget> interface rather than using a fixed Sleep period to minimize
 *                                              the delay in polling the VCU.
 ...
 * 
 *  11/17/10    6.4.0.4     K.McD           1.  Replaced the WorksetManager static class with the combination of: (a) the WorksetCollection class, a non-static class
 *                                              used to manage the workset collections associated with individual sub-systems and (b) the Workset class, a static class
 ...
 *                                          2.  Replaced the 'Configure/Worksets' menu option with the 'Configure/Worksets/Watch Window' and 'Configure/Worksets/Fault
 *                                              Log' menu options.
 ...
 *                                              before updating the data stream parameters. If this is not carried out, any attempt to view a data stream generated
 *                                              before the parameters were modified will display incorrect results as the values will correspond to the watch identifiers
 ...
 *                                                  (b) Modified the PathRelativePTUApplicationData resource in Common.dll as a temporary fix so that the default
 *                                                      directory for PTU application data is 'ProgramData/Bombardier/Portable Test Unit'.
 *                                                  (c) Modified the GetStream() method of the Event.CommunicationEvent class to set the data stream number to be equal
 *                                                      to the event record index + 1.
 * 
 *  01/12/11    6.4.2       K.McD           1.  Bug fix SNCR001.84. Added the second parameter to all calls to the System.Threading.WaitHandle.WaitOne() method, as
 *                                              advised by the following blog entries: 
 ...
 *                                              whenever an event record is found that has an associated data stream, the stream number for the event record is
 *                                              incremented from a starting value of 0. Any event record that does not have an associated data stream will have the
 *                                              stream number set to CommonConstants.NotUsed.
 * 
 ...
 * 
 *                                          6.  Modified the form used to display and record watch variables, Watch.FormWatchView, so that: (a) the escape key is no
 *                                              longer disabled while in record mode (b) access to the main menu options is disabled while in record mode and (c) if the 
 ...
 * 
 *                                          4.  Modified the Common.FormDataStreamReplay class to display the appropriate text and image on the escape key ToolStripButton
 ...
 *                                   
 *                                          7.  Modified the event handler for the 'Set As Default' context menu option associated with the Common.FormWorksetManager
 *                                              class to ensure that the security level of the workset that has been set is assigned to the highest security level.
 * 
 ...
 *                                              workset.
 * 
 *                                          9.  Bug fix SNCR001.105. Added the UpdateMenu() override method to the FormViewWatch class to process any form specific
 *                                              changes to the menu system or function keys resulting from a change in the security level of the user.
 * 
 ...
 * 
 *                                          12. Bug fix. Modified the CheckFaultlogger() method of the Event.CommunicationEvent class such that the newIndex local
 *                                              variable used in the call to the PTUDLL32.CheckFaultLogger() method in not initilalized to zero.
 * 
 ...
 * 
 *                                          16. Modified the event handler for the Save function key Click event of the Event.FormSetupStream class to ask for
 ...
 *                                              methods.
 *                                          
 *                                          2.  Modified the event handler for the 'F5-Trip' function key 'Click' event in the Common.FormDataStreamReplay class such
 *                                              that, if an entry corresponding to the time of the trip is not found, the 'F5-Trip' function key is unchecked and the
 *                                              cursor is set to the default cursor.
 * 
 ...
 *                                              (e) FormWorksetDefine, Watch.FormWorksetDefineWatch, Watch.FormWorksetDefineChartRecorder,
 *                                                  Event.FormWorksetDefineFaultLog.
 ...
 *                                          Enabled property of the control is not asserted.
 *                                          
 *                                      7.  Updated SelfTest.dll (Ver. 1.3.11). Bug fix - SNCR002.41. Added checks to the F2 event handler associated with the
 *                                          FormTestView class to ensure that the event handler code is ignored if the Enabled property of the control is not asserted.
 *                                          
 ...
 *                                              							
 *	11/14/11	6.6.23		Sean.D		1.	Updated Common.dll and GraphComponents to handle SNCR002-03 (Memory leak when opening multiple watch files) which had
 *	                                        recurred.
 ...
 *                                                      (a) Ensured that all event handler methods were detached and that the component designer variables were set to
 *                                                          null on disposal.
 ...
 *                                                  4.  Updated DataDictionary.xsd to reflect the modified structure of the DataStreamType table. Note: Automatic update
 *                                                      of DataDictionary.Designer.cs.
 ...
 *                                              VCU.
 *                                              
 *                                          2.  Now displays the number of watch variables that are associated with the current event log on the form used to configure
 *                                              the fault log parameters.
 *                                          
 ...
 *                                              record 20 watch variables whereas the propulsion and engineering logs can only record 16, the form only displays the
 *                                              number of watch variables that have been configured so far. For such projects the PTU does not know whether the user is
 *                                              defining a workset for the snaphot, propulsion or engineering event log and it is the responsibility of the user to
 *                                              define the correct number of watch variables for the workset.
 *                                              
 ...
 *                                              current XML data dictionary file rather than the default XML data dictionary file. The current XML data dictionary
 *                                              filename can specify either: (a) the default XML data dictionary file or (b) the project XML data dictionary file.
 *                                          
 ...
 *                                      and Windows 7 operating systems. The default/initial font for the Windows XP setting was set to Tahoma, 8pt and the default/
 *                                      initial font for the Windows 7 setting was set to Segoe UI - 8.25pt. These fonts are the default fonts for Windows XP and Windows
 *                                      7 respectively.
 *                                      
 ...
 *                                      value directly from the Font setting, now calls the FormOptions.GetOSFont() static method to get the font setting (FontXP/
 *                                      FontWin7) that is appropriate to the operating system.
 *                                  
 ...
 *                                          
 *                                          3.  Modified the FormPTU class to change the text associated with the escape key from 'Esc - Home' to 'Esc' to prevent
 ...
 *                                          and ensure that the text indicates 'disconnect'. If true, no further action is required.  If it indicates 'connect', then
 *                                          click' connect' and the network adapter will be enabled.'.
...
#region - [Revision History 6.16.2] -
/*
 *  11/16/15    6.16.2    K.McD     References
 ...
 *
 *                                  Modifications
 *                                  1.  Resources.resx. Added TextProjectTitleR179 and TextProjectTitleR188 strings. - Ref.: 1.
 *                                  2.  MenuInterfaceApplication.cs. Rev. 1.14. - Ref.: 1.
 *                                  3.  MdiPTU.Support.cs. Rev. 1.23. - Ref.: 1.
 *                                  4.  FormHelpAbout.cs.Rev. 1.4. - Ref.: 1.
 *                                  5.  ControlPanel.cs. Rev. 1.4. ControlPanel.Designer.cs. Rev. 1.4. - Ref.: 1.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.27. - Ref.: 1, 2, 3.
 */
#endregion - [Revision History 6.16.2] -

#region - [Revision History 6.16.3] -
/*
 *  03/02/16    6.16.3      DAS     References
 ...
 *                                      
 *                                  2.  Bug Fix - SNCR - R188 [20-Mar215] Item 37. Fixed issues associated with logging and displaying real time events while
 ...
 *  
 *                                  Modifications
 *                                  1.  \Communication\CommunicationApplication.cs Rev. 1.7. - Ref.: 1.
 *                                  2.  FormSelectTarget.cs Rev. 1.6. - Ref.: 1.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Replaced the unmanaged VcuCommunication Application Extension Assembly Rev. 2.2 with the VcuComm Application Extension Assembly
 *                                      Rev. 1.0. - Ref.: 1.
 *                                  2.  Common Application Extension Assembly. Rev. 1.28. - Ref.: 1.
 *                                  3.  Watch Application Extension Assembly. Rev. 1.28. - Ref.: 1.
 *                                  4.  Event Application Extension Assembly. Rev. 1.26. - Ref.: 1, 2.
 *                                  5.  SelfTest Application Extension Assembly. Rev. 1.8. - Ref.: 1.  
 *                                  
 *                                  File Changes
 *                           
 *  FILE NAME                                                   STATUS   
 ...
 * 
 */
#endregion - [Revision History 6.16.3] -

#region - [Revision History 6.16.4] -
/*
 *  03/15/16    6.16.4              References
 *                          K.McD   1.  PTU Modifications to Support the Requirements Defined in the San Francisco Bay Area Rapid Transport (BART) PTU Generic
 ...
 *                                          
 *                          DAS     2.  Bug Fix - SNCR - PTU [01 Mar 2016] - Item 8. The timeGetTime() function of winmm.dll, which is used in the PollScheduler
 *                                      class, has potential to cause problems when running the PTU on 64 bit machines. Replace this functions with
 *                                      the .NETSystem.Environment.TickCount property instead.
 *                                      
 *                          DAS     3.  Bug fix - SNCR - PTU [01 Mar 2016] - Item 9. Whenever communication type is set to BOTH (serial and TCP) in the
 *                                      configuration XML file, and a serial port doesn't exist on the PC (no comm port in Device Manager), the PTU doesn't attempt to
 *                                      poll potential TCP connections.
 *                                      
 *                          K.McD   4.  Bug fix - SNCR - PTU [01 Mar 2016] - Item 12. There is an issue with saving fault/event logs on BART. BART has car IDs that are
 ...
 *                                      
 *                                          if (eventLogFile.EventRecordList[0] != Settings.Default.MostRecentDownloadedEventsSaved.DownloadedEvent[MainWindow.CarNumber])
 *                                          {
 *                                              ...
 *                                          }
 *                                          
 ...
 *
 *                                  
 *                          DAS     5.  Bug fix - SNCR - PTU [01 Mar 2016] - Item 13. When viewing the Event Screen, the Maintenance log always downloads and updates
 ...
 *                                        
 *                                  Modifications
 *                                  1.  FormSelectTarget.cs. Rev. 1.7. - Ref.: 3.
 *                                  2.  MdiPTU.Support.cs. Rev. 1.24. - Ref.: 4.
 *                                  3.  MdiPTU.cs. Rev. 1.20. - Ref.: 4.
 *                                  4.  CommunicationApplicationOffline.cs. Rev. 1.1. - Ref.: 1.
 *                                  5.  Corrected the date associated with the Rev. 6.16.3 revision.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.29. - Ref.: 1, 2, 4, 5.
 *                                  2.  Event Application Extension Assembly. Rev. 1.27. - Ref.: 1.
 *                                      
 */
#endregion - [Revision History 6.16.4] -

#region - [Revision History 6.16.5] -
/*
 *  03/31/16    6.16.5      K.McD   References
 ...
 *                                      
 *                                  2.  Bug Fix - SNCR-PTU [01 Mar 2016] - Item 11. There appears to be a problem with the SetTimeDate() method in the
 ...
 *                                  
 *                                  4.  Bug Fix - SNCR - PTU [01 Mar 2016] - Item 15. While testing out the Csv file generation using the CTA VCU the CarID property
 ...
 *                                      
 *                                  5.  Bug Fix - SNCR-PTU [01 Mar 2016] - Item 18. Bug Fix - SNCR-PTU [01 Mar 16] Item 18. While testing the changes associated with
 *                                      items 10, 11, 14 and 15 a number of valid fault datastreams were generated for the CTA project using the following procedure:
 *                                      
 ...
 *                                  
 *                                  6.  Bug Fix - SNCR-PTU [01 Mar 2016] - Item 19. Release 6.16.4 of the PTU can't access fault data streams that have been generated
 ...
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.30. - Ref.: 1, 3, 4, 5, 6.
 *                                  1.  VcuComm Application Extension Assembly. Rev. 1.1. - Ref.: 1, 2, 3.
 *                                      
 */

/*
 *  04/07/2016  6.16.5.1            References
 ...
 *                                      
 *                                  Modifications
 *                                  1.  MdiPTU.Support.cs. Rev. 1.25. - Ref.: 1. 
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.31. - Ref.: 1. 
 *                                  2.  SelfTest Application Extension Assembly. Rev. 1.9. - Ref.: 1.
 */

/*
 *  04/07/2016  6.16.5.2            References
 ...
 *                                      
 *                                  3.  Conference Call 11th April 2016 - Add Control Panel to the BART and Toronto Rocket projects.
 *                                      
 *                                  Modifications
 *                                  1.  MdiPTU.Support.cs. Rev. 1.26. - Ref.: 3.
 *                                  2.  Modified Resources.resx. Added the Control Panel title text for the BART and Toronto Rocket projects.
 *                                  3.  Modified ControlPanel.cs. Rev. 1.5, ControlPanel.Designer.cs. Rev. 1.5. - Ref.: 3.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.31.1 - Ref.: 1, 2. 
 *                                  2.  Event Application Extension Assembly. Rev. 1.27.1. - Ref.: 1.
 */

/*
 *  04/21/2016  6.16.5.3            References
 *                                  1.  SELFTESTERRMESS table of TOPC3607.E1.
 *                                  2.  Bug Fix - SNCR-PTU [01 Mar 2016] - Item 20. If the user tries to change the font to Microsoft JengHei, an ArgumentException
 *                                      is thrown with the error message ‘Only TrueType fonts are supported. This is not a TrueType font’.
 *  
 *                                  Modifications
 *                                  1.  FormOptions.cs. Rev. 1.6. - Ref. 2.
 *                                  2.  Resources.resx. Added MBTNotTrueTypeFont string resource. - Ref.: 2.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  SelfTest Application Extension Assembly. Rev. 1.9.1. - Ref.: 1.
 */
#endregion - [Revision History 6.16.5] -

#region - [Revision History 6.17] -
/*
 * 05/12/2016   6.17        K.McD   References
 ...
 * 
 *                                  Modifications
 *                                  1.  MdiPTU.cs. Rev. 1.21. - Ref.: 1.
 *                                  2.  MdiPTU.Support.cs. Rev. 1.27. - Ref.: 1.
 *                                  3.  Resources.resx. - Ref.: 1.
 *                                      1.  Added: MBTConfigProjectIDPassedAsParameterNotFound, MBTConfigSecurityLevelPassedAsParameterInvalid and
 *                                          MBTSecurityCurrentUserNotGroupMember.
 *                                      2.  Renamed: MBTConfigProjectAsParameterMismatch to MBTConfigProjectIDMismatch.
 *                                      3.  Renamed: MBTConfigInvalid to MBTConfigDataDictionaryInvalid.
 *                                      4.  Renamed: MBTConfigProjectNotFound to MBTConfigDataDictionaryDoesNotExist.
 *                                      5.  Renamed: MBTConfigDefaultNotFound to MBTConfigDataDictionaryDefaultDoesNotExist.
 *                                      6.  Deleted: MBTShortcutParameterCountInvalid.
 *                                      7.  Deleted: MBTConfigReselect.
 *                                  4.  Program.cs. Rev. 1.3. - Ref.: 1.
 *                                      
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.32 - Ref.: 1, 2.
 */
#endregion - [Revision History 6.17] -

#region - [Revision History 6.18] -
/*
 * 05/12/2016   6.18        DAS     References
 ...
 *                                  5.  PTE Changes - List 5-17-2016.xlsx Item 1. Login Screen - Make the Bombardier logo smaller.
 *                                  6.  PTE Changes - List 5-17-2016.xlsx Item 2. Login Screen - Add 'Portable Test Application' with the version of the PTU software.
 *                                  7.  PTE Changes - List 5-17-2016.xlsx Item 3. Login Screen - Add OK and Cancel buttons.
 *                                  8.  PTE Changes - List 5-17-2016.xlsx Item 69. Login Screen - Add 'Enter factory or engineering password.' above the Password: text
 *                                      entry box.
 *                                  9.  PTE Changes - List 5-17-2016.xlsx Item 5.   Home Screen - Change 'F2-Select Target Logic' ToolStripButton Text to be
 *                                      'F2-Connect to Target.
 *                                  10. PTE Changes - List 5-17-2016.xlsx Items: 36, 37, 38. When the: 'System SW Version', 'Log Erasure' or 'Password Protection'
 ...
 *                                  11. PTE Changes - List 5-17-2016.xlsx Items: 7, 8. Under the 'File/Open' menu option, remove the line between the 'Watch File' and
 *                                      'Simulated Data Stream' menu options and reorder the list as follows: Watch File, Event Log, Data Stream, Simulated Data Stream.
 *                                  12. PTU Conference call Thu 7th July 2016. Request to Change the error message text when there is a mismatch between the PTU data
 ...
 *                                  13. PTE Changes - List 5-17-2016.xlsx Item 26. Do not include the individual TabPage 1 Count value for the DataStream
 *                                      Configuration screen, just show the Total Count value.
 *                                  14. PTE Changes - List 5-17-2016.xlsx Item 43 and 65. Home / Event Log Screens - After selecting the 'F5-Init' ToolStripButton on
 *                                      the Event Log Screen or the 'Log Erasure' button on the Control Panel/Home Screen, add the text 'Initializing event logs
 *                                      will reset all flags to default, clear all cumulative history, and erase all event logs.' prior to the 'Are you sure…?' message.
 *                                      
 *                                  Modifications
 *                                  1.  FormLogin.cs. Rev.1.3. - Ref.: 7. FormLogin.Designer.cs. Rev. 1.2. - Ref.: 5, 6, 7, 8.
 *                                  2.  FormSelectTarget.cs. Rev. 1.8. - Ref.: 9, FormSelectTarget.Designer.cs. Rev. 1.2. - Ref.: 9.
 *                                  3.  ControlPanel.cs. Rev. 1.6. - Ref.: 10.
 *                                  4.  Resources.resx.
 *                                      1.  Split the MBTConfigVCUMismatchVersion resource string into 3 components.
 *                                      2.  Changed FunctionKeyTextSelectLogic string resource to 'F2-Connect to Target' and FunctionKeyTextDisconnectLogic string
 *                                          resource to 'F2-Disconnect Target'. - Ref.: 9.
 *                                  5.  MdiPTU.cs. Rev. 1.22. - Ref.: 12, MdiPTU.Designer.cs. Rev. 1.13. - Ref.: 11.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.33 - Ref.: 1, 2, 3, 4.
 *                                  2.  Watch Application Extension Assembly. Rev. 1.29. - Ref.: 1, 13.
 *                                  3.  SelfTest Application Extension Assembly. Rev. 1.9.2 - Ref.: 2, 13.
 *                                  4.  Event Application Extension Assembly. Rev. 1.27.2. - Ref.: 1, 13, 14.                                    
 */

/*
 *  07/20/2016  6.18.1      K.McD   References
 *                                  1.  PTE Changes - List 5-17-2016.xlsx Item 4. Monitor the Wibu-Key continuously while not connected to the target logic.
 *                                  2.  PTE Changes - List 5-17-2016.xlsx Item 6. When exiting the application, prompt the user with an alert 'Are you sure that you want
 *                                      to exit the Portable Test Application?'. Use 'Yes' or 'Cancel' buttons for selection.
 *                                  3   PTE Changes - List 5-17-2016.xlsx Item 10, 49, 58. The 'Configure/Enumeration' drop-down menu option is to be replaced by
 *                                      individual ToolStripButton controls on the 'Watch Window', 'Event Log' and 'Self Test' screens.
 *                                  4.  Bug Fix - SNCR - PTU [01 Mar 2016] - Item 21. When displaying enumerator variables the display should toggle between the raw
 *                                      integer values and the equivalent text strings as soon as the user selects the 'Enumeration' ToolStripButton.
 *                                  5.  PTE Changes - List 5-17-2016.xlsx Item 11. Move the 'Real Time Clock' menu option from the 'Configure' drop-down menu to the
 *                                      'Tools' drop-down menu.
 *                                  6.  PTE Changes - List 5-17-2016.xlsx Item 12. Under the 'Help' drop-down menu, add a 'Show User Manual menu option above the 'About'
 *                                      menu option with a link to the 'Software User Manual'.
 *                                  7.  PTE Changes - List 5-17-2016.xlsx Item 13, 27, 34. When configuring the Fault Log DataStream, Chart Recorder or Watch Window,
 *                                      the user must only be allowed to select variables from either the 'Available Watch Variables' list or the 'Workset Layout' list
 *                                      but not both at the same time.
 *                                  8.  PTE Changes - List 5-17-2016.xlsx Item 56. Remove the 'F4-Pause' ToolStripButton from the 'Watch Window' screen.
 *                                  9.  [TBC] PTE Changes - List 5-17-2016.xlsx Items 16, 24, 61.  For the Chart Recorder, Data Stream and Watch Window configuration
 *                                      dialog boxes, change the word 'Workset' to 'Chart Recorder Configuration', 'Data Stream Configuration' and 'Watch Window
 *                                      Configuration' respectively and change to title of each dialogbox to be 'Configure - Chart Recorder', 'Configure - Data Streams'
 *                                      and 'Configure - Watch Window'.
 *                                  10. PTE Changes - List 5-17-2016.xlsx Item 18. Use a double-click to change the chart scale factor on the
 *                                      'Chart Recorder Configuration' dialog box.
 *                                  11. [TBC] PTE Changes - List 5-17-2016.xlsx Item 44. On the 'Download Event History' dialog box, remove the greyed-out 'Cancel'
 *                                      button.
 *                                  12  [TBC] PTE Changes - List 5-17-2016.xlsx Item 45. On the 'Event History' dialog box, replace the 'Cumulative History' and 'Recent
 *                                      History' columns with a single 'Event History' column that gives the combined value of both cumulative and recent values.
 *                                  13. [TBC] PTE Changes - List 5-17-2016.xlsx Item 59. On the 'Watch Window' screen change the name of the F5 ToolStripButton control to
 *                                      'F5-Configure'.
 *                                  14. PTE Changes - List 5-17-2016.xlsx Item 62. On all screens, change the green text to black.
 *                                  15. PTE Changes - List 5-17-2016.xlsx Item 63. Change the '(Bit Mask)' component of the Bitmask UserControl to say 'Detail'
 *                                      and make it look like a clickable button.
 *                                  16  PTE Changes - List 5-17-2016.xlsx Item 64. When double-clicking on the '(Bit Mask)' component of the Watch Variable
 ...
 *                                  17. PTE Changes - List 5-17-2016.xlsx Item 67. In the 'Workset' drop-down menu on the right side of the 'View/Watch Window' screen,
 *                                      the order in which the worksets are arranged changes whenever a workset is selected or the application is opened/closed. This
 *                                      order should be permanently fixed.
 *                                  18. [TBC] PTE Changes - List 5-17-2016.xlsx Item 66. The “/” and other characters that are invalid for a filename cause certain events
 ...
 *                                  19. Bug Fix - SNCR-PTU [01 Mar 2016] - Item 22. On the event screen, modify the width of the event variable user control to allow
 *                                      for a vertical scroll bar.
 *                                      
 * 
 *                                  Modifications
 *                                  1.  MdiPTU.Designer.cs. Rev. 1.14. - Ref.: 2, 3, 5, 6.
 *                                  2.  MdiPTU.cs. Rev. 1.22.1. - Ref.: 1, 5, 6.
 *                                  3.  MdiPTU.Support.cs. Rev. 1.27.1. - Ref.: 1, 5.
 *                                  4.  MenuInterfaceApplication.cs. Rev. 1.15. - Ref.: 5.
 *                                  5.  Renamed FormConfigureRealTimeClock to FormRealTimeClock. - Ref.: 5.
 *                                  6.  Resources.resx. Added the MBTQueryExit string resource. - Ref.: 2.
 *                                  7.  ControlPanel.cs. Rev. 1.6.1. - Ref.: 6.
 *                                                          
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.31.1. - Ref.: 3, 4, 6, 7, 14, 15, 16.
 *                                  2.  Watch Application Extension Assembly. Rev. 1.29.1. - Ref.: 8, 10, 17.
 *                                  3.  SelfTest Application Extension Assembly. Rev. 1.9.3 - Ref.: 14.
 *                                  4.  Event Application Extension Assembly. Rev. 1.27.3. - Ref.: 4, 14, 16, 19.
 *                                  
 *                                  Note:   [TBC] - To Be Completed. These software modifications are to be included in the R188 Release targeted for 3rd Aug 2016,
 *                                          however, they are still to be completed. This is an interim release to allow PIPPC to test all R188 modifications carried
 *                                          out so far.
 *                                         
 */

/*
 *  07/20/2016  6.18.2      K.McD   References
 ...
 *                                      
 *                                  2.  PTE Changes - List 5-17-2016.xlsx Item 44. On the 'Download Event History' dialog box, remove the greyed-out 'Cancel'
 *                                      button.
 *                                      
 *                                  3.  PTE Changes - List 5-17-2016.xlsx Item 45. On the 'Event History' dialog box, replace the 'Cumulative History' and 'Recent
 *                                      History' columns with a single 'Event History' column that gives the combined value of both cumulative and recent values.
 *                                      
 *                                  4.  PTE Changes - List 5-17-2016.xlsx Items 16, 24, 61.  For the Chart Recorder, Data Stream and Watch Window configuration
 *                                      dialog boxes, change the word 'Workset' to 'Chart Recorder Configuration', 'Data Stream Configuration' and 'Watch Window
 ...
 *                                      
 *                                  5.  PTE Changes - List 5-17-2016.xlsx Item 59. On the 'View/Watch Window' and 'Diagnostics/Self-Tests' screens, change the name of the
 ...
 *                                      
 *                                  6.  PTE Changes - List 5-17-2016.xlsx Item 66. The “/” and other characters that are invalid for a filename cause certain events
 *                                      not to be saved. The Data Dictionary should be changed to remove any invalid characters OR the software should look for these
 *                                      characters and replace them prior to saving.
 *                                      
 *                                  7.  PTE Changes - List 5-17-2016.xlsx Item 2. Add "Portable Test Application" with the version of the PTU Software to the Login
 *                                      screen.
 *                                      
 *                                  8.  PTE Changes - List 5-17-2016.xlsx Item 69. Add 'Enter Factory or Engineering Password' above the 'Password' text entry box. This
 *                                      should be project specific i.e. for the CTA and Toronto projects the text should read 'Enter Factory, Engineering or Maintenance
 *                                      Password'.
 *  
 *                                  Modifications
 *                                  1.  FormLogin.cs. Rev. 1.4, FormLogin.Designer.cs. Rev. 1.3. - Ref.: 7, 8.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.31.3. - Ref.: 1, 5, 6.
 *                                  2.  SelfTest Application Extension Assembly. Rev. 1.9.4 - Ref.: 1, 5.
 *                                  3.  Event Application Extension Assembly. Rev. 1.27.4. - Ref.: 2, 3, 4, 5.
 *                                  4.  Watch Application Extension Assembly. Rev. 1.29.2. - Ref.: 5.
 */

/*
 *  08/03/2016  6.18.3      K.McD   References
 ...
 *                                      
 *                                  2.  Bug Fix. With reference to the email from Dave Smail to Keith McDonald on 09-Aug-2016 16:57 BST - Subject: PTU Solution Update.
 *                                      There is an issue with the GetChartMode() method in the WatchClockMarshal class.
 *                                      
 *                                  3.  PTE Changes - List 5-17-2016.xlsx Item 15, 22, 23, 23, 25, 47, 48. Add 'Delete', 'Set As Default' and 'Override Security'
 ...
 *                                      
 *                                  4.  PTE Changes - List 5-17-2016.xlsx Item 9. Under the Configure drop-down menu, only allow the following selections: Watch Window,
 *                                      Chart Recorder and Data Stream.
 *                                      
 *                                  5.  PTE Changes - List 5-17-2016.xlsx Item 19, 20, 28. Make the Chart Recorder, Data Stream and Watch Window configuration dialogbox
 *                                      available from the 'Configure' drop-down menu and remove the 'Manage' window completely.
 *                                      
 *                                  6.  Bug - Self test results screen locked up when running interactive test.
 *                                  
 *                                  7.  Bug Fix - Support 4 digit year code returned from VCU.
 *                                  
 *                                  8.  Bug Fix - Ref. Email Atul Chaudhari 16 Aug 2016. When using the ProcessStartInfo class to pass parameters to the Process.Start()
 ...
 *                                      
 *                                  9.  Bug Fix - The contents of the 'Notes' GroupBox control on ALL dialogboxes derived from the 'Workset Manager' form cannot be
 *                                      fully seen as the size of the dialogboxes is too small.
 *                                      
 *                                  10. Bug Fix - SNCR-PTU [01 Mar 2016] - Item 23. Only ask the user if they wish to terminate the application if they select
 *                                      the 'File/Exit' menu option or select the [x] button. If the close is as a result of a fatal error just close the program
 *                                      regardless.
 *                                  
 *                                  Modifications
 *                                  1.  MdiPTU.cs. Rev. 1.22.2. - Ref.: 1, 10.
 *                                  2.  MdiPTU.Designer.cs. Rev. 1.15. - Ref.: 1, 4, 5, 10.
 *                                  3.  MdiPTU.Support.cs. Rev. 1.27.2. - Ref.: 3, 10.
 *                                  4.  FormHelpAbout.cs. Rev. 1.5. - Ref.: 8.
 *                                  5.  MenuInterfaceApplication.cs. Rev. 1.16. - Ref.: 8.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.33.5. - Ref.: 2, 3, 6, 7, 10.
 *                                  2.  Watch Application Extension Assembly. Rev. 1.29.3. - Ref.: 1, 3, 5, 9.
 *                                  3.  SelfTest Application Extension Assembly. Rev. 1.9.5 - Ref.: 6.
 *                                  4.  Event Application Extension Assembly. Rev. 1.27.6. - Ref.: 3, 5, 7, 9.
 */

/*
 *  10/10/2016  6.18.4      K.McD   References
 *                                  1.  Request by Atul Chaudhari - Email: Sue Strane 10th Oct 2016 10:07. Remove all whitespace from the filenames associated with
 *                                      the project 'Software User Manual' and 'Release Notes' e.g. 'R8PR.Portable Test Equipment - Software User Manual.pdf is
 *                                      replaced by 'R8PR.PTE-SoftwareUserManual.pdf'.
 *                                      
 *                                  2.  Ref.: Conference Call 3rd Oct 2016. For both BART and R188, the fields of the CSV file that is produced whenever and new event
 *                                      log XML file is generated should should be based upon document 'NYCT R188 - Variation Order (14th Jan 2016) - XML to CSV.pdf'.
 *                                                
 *                                  Modifications
 *                                  1.  Resources.resx. Renamed the 'Release Notes' and 'Software User Manual' filenames to: 'PTU-ReleaseNotes'/
 *                                      'PTE-ReleaseNotes' and 'PTU-SoftwareUserManual'/'PTE-SoftwareUserManual' depending upon whether the project refers to
 *                                      the application as Portable Test Unit or Portable Test Equipment. Also renamed the BART 'Software User Manual' filename to 
 *                                      'BART.PTU-SoftwareUserManual.chm'. - Ref.: 1.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.33.6. - Ref.: 2.                            
 * 
 */

/*
 *  10/14/2016  6.18.5      K.McD   References
 ...
 *                                                
 *                                  Modifications
 *                                  1.  Resources.resx. Modified the text associated with the MBTSecurityCurrentUserNotGroupMember and
 *                                      MBTConfigSecurityLevelPassedAsParameterInvalid string resources. - Ref.: 1.
 *                                      
 *                                  2.  MdiPTU.cs. Rev. 1.23.1. - Ref.: 1.
 * 
 */

/*
 * 12/09/2016   6.18.6      vgott   References
 *                                  1.  The below tasks from sharepoint portal are implemented.
 *                                      1.  Change "Log Erasure" button to "Clear Event Log"
 *                                      2.  Button "Log" changed to "Event Log"
 *                                  
 *                                  Modifications
 *                                  1.  Button "Log Erasure"  to "Clear Event Log"
 *                                  2.  Button "Log" changed to "Event Log"
 *                                  
 *                          K.McD   References
 *                                  1. Internal audit of the code prior to handover to India.
 *                                  
 *                                  Modifications
 *                                  1. Removed the old VcuCommunication32 and VcuCommunication64 dynamic link libraries from the repository as these have been replaced by 
 *                                  VcuComm.dll, a managed version of the communication DLL. *                                  
 *                          
 */
/*
 * 02/07/2017   6.18.7      vgott   References
 *                                  1. Change "Password Protection" button to "Password Manager"
 *                                  2. Add the text “PC” to “Date” in Data Monitor Window and in the Event Window.
 *                                  3. Change the word “Set” to “Enabled” and “Reset” to “Disabled” in "Configuration Event Flags" dialog box from Event Log Window. 
 *                                  
 *                                  Modifications
 *                                  1. Button "Password Protection"  to "Password Manager"
 *                                  2. Updated the text associated with InformationLegendDate in Resources.resx files of Watch.dll and Event.dll.
 *                                  3. Updated the text associated with TextReset and TextSet in Resources.resx file of Event.dll.
 *                                  
 *                          
 */
/*
 * 02/07/2017   6.18.8      vgott   References
 *                                  1. Change the header columns text as "Event Trigger" and "Stream Trigger" of Configure Event Flags dialogbox.
 * 
 *                                  Modifications
 *                                  1. Updated the Header text of columns in data grid view with "Event Trigger" and "Stream Trigger".
 *                                  
 *                          
 */
/*
 * 02/21/2017   6.18.9      vgott   References
 *                                  1. Update the tooltip text format by removing '[' and ']' characters.
 *                                  2. Enable the tooltip for chart mode options.
 *                                  3. Update the existing tooltip text in Watch.dll, Event.dll and SelfTest.dll and Common.dll as suggested in "Tooltip Analysis on PTU App-ANC_02162017.docx"
 *                                  4. Update the tooltip text format by adding '[' and ']' characters to functionkeys only and keys are at the end of the text.
 * 
 *                                  Modifications
 *                                  1. Added tooltip text  for all buttons in Home Window.
 *                                  2. Added tooltip text  for all chart mode option in Watch.dll.
 *                                  3. Update the tooltip text format for all tooltips in Resource.resx files of PTU , Watch.dll, Event.dll and SelfTest.dll and Common.dll.
 *                                  4. Update the tooltip text format and chages done in the DisplayFunctionKey() for suggested tooltip format.
 *                                  
 *                          
 */
#endregion - [Revision History 6.18] -

#region - [Revision History 6.19] -
/*
 * 05/12/2016   6.19.1.0  D.Smail   Modifications
 *                                  1.  Include changes from 6.18.6.0 - 6.18.9.0 
 *                                  2.  After connecting to the target hardware, the software now "pings" the target hardware
 *                                      on the Main form and informs the user when communication is lost
 *                                  3.  The software "pings" the tarfet software on the self test form even when any 
 *                                      self test isn't executing.
 *                                  4.  Via the FunctionFlags bit 3, the user can enable a check if the target hardware 
 *                                      is still in self test. This requires a change to the target hardware and therefore 
 *                                      has been made optional (project configuration file)
 *                                  5.  Fixed issue with Windows execption being declared if communication is lost between the 
 *                                      time a target is selected and the time the user clicks OK.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.34.1.0 
 *                                  2.  Event Application Extension Assembly. Rev. 1.28.1.0
 *                                  2.  SelfTest Application Extension Assembly. Rev. 1.10.1.0
 *                                  3.  VcuComm Application Extension Assembly. Rev. 1.2.1.0
 *                                  4.  Watch Application Extension Assembly. Rev. 1.30.1.0                                
 */
/*
 * 06/05/2017   6.19.2.0  D.Smail   Modifications
 *                                  1.  Removed Console.Writelines() that were used for debug purposes only. No change to functionality
 *                                  2.  The "Cancel" button text to "Close" for all "Configure" forms. Having the text "Cancel" implies that the none of the user changes would have been saved
 *                                  3.  A change was made to the code to disable the Windows Timer from firing; therefore no events were palced on the UI queue. 
 ...
 */
/*
 *  06/12/2017   6.19.2.1  D.Smail   Modifications
 *                                  4.  PTE Changes - List 5-17-2016.xlsx Item 5. Home Screen - Change 'F2-Select Target Logic' ToolStripButton Text to be'F2-Connect to Target'.
 *                                  5.  For BART project only, the displayed URL is now a user friendly URL that lists the car Id, consist and truck type.
 *                                  6.  "OK" button now available as soon as the first TCP target is detected. Prior to this change all
 *                                          targets had to be scanned before the "OK" button was made available
 *                                                              
 */
/*
 * 06/24/2016   6.19.3.0  Vgottam   Modifications
 *                                  1.  Added the ControlPanel Load event. To display the PTU status panel header as "PTU" for BART and  "PTE" for other projects.
 *                                  2.  Disabled  the m_MenuItemConfigurePasswordProtection menu item for password protection for BART project.
 *                                  3.  Updated the tooltips text for F8-Stream , F6-Flags and all F5-Config,F1-Enum buttons.
 *                                  4.  Added truck type parameter as as empty to DeriveName() method if the default file structure is other than EventLogs.
 *                                  5.  Get the truck type from the event record list to and passed the truck type parameter to DeriveName() for the default file structure of EventLogs.
 *                                  6.  Converted CarID to upper which is recieved from m_RxMessage of request type "GET_EMBEDDED_INFORMATION"
 *                                  7.  Tooltip update for Help button as  “Access to PTU(E) Help" instead of "Software User Manual”.
 *                                  8.  Updated the About Form text to display the "Portable Test Unit(PTU)" for BART and "Portable Test Equipment(PTE)"  for other projects.
 *                                  
 *                                  Dynamic Link Library Updates
 *                                  1.  Common Application Extension Assembly. Rev. 1.34.2.0 
 *                                  2.  Event Application Extension Assembly. Rev. 1.28.2.0
 *                                  3.  SelfTest Application Extension Assembly. Rev. 1.10.2.0
 *                                  4.  Watch Application Extension Assembly. Rev. 1.30.2.0
 *                                                              
 */
/*
 * 09/06/2017   6.19.3.1  D.Smail   Modifications
 ...
 *                                  2. Modified m_CommTimer_Tick() so that after initialization the state machines transitions to the Pause state instead 
 *                                     of the Poll state. This change was made to support CTA immediately transitioning to Watch screen instead of Home (like 
 *                                     all other projects).
 *                                  3. Made changes to m_TSBOnline_Click() and m_TSBOffline_Click() to account for CTA directly transitioning to Watch screen instead of Home screen. 
 *
 *                                  Dynamic Link Library Updates 
 *                                  1.  VcuComm Application Extension Assembly. Rev. 1.3.1.0
 */
/*
 * 09/12/2017   6.19.3.1  Vgottam   Modifications
 *                                  1.  Disabled Menus of main window,Menu: Diagnostics and pull down menu item: View->WatchWindow.
 *                                  
 *                                  Dynamic Link Library Updates 
 *                                  1.  Event Application Extension Assembly. Rev. 1.28.3.0
 *                                  2.  SelfTest Application Extension Assembly. Rev. 1.10.3.0
 *                                  3.  Watch Application Extension Assembly. Rev. 1.30.3.0
 *                                                              
 */
/*
 * 09/15/2017   6.19.3.1  D.Smail   Modifications
 *                                    1.  Added some delays between self test commands (in FormViewTestResults()) when entering self test to support 
 *                                        Windows XP issue for CTA (serial). When using the PTU on an XP machine for CTA only, entering
 *                                        self test properly is marginal. We believe the problem is on the target side, but
 *                                        an investigation may take weeks. This "band-aid" solves CTA's issue, but has no side effects
 *                                        on other projects (either serial or TCP).
 *                                     2. Added a delay in StartSelfTestTask() to support Windows XP issue for CTA (serial). When using 
 ...
 *                                  
 *                                  Dynamic Link Library Updates 
 *                                  1.  Common Application Extension Assembly. Rev. 1.34.3.0
 *                                  2.  SelfTest Application Extension Assembly. Rev. 1.10.3.1
 *                                                              
 */
/*
 * 11/06/2017   6.19.4.0  Vgottam   Modifications
 *                                  1.  CTA 5000 - PTU issue with delay time and updated "DefaultMutexWaitDurationMs" with 120 seconds.
 *                                  
 *                                  Dynamic Link Library Updates 
 *                                  1.  Common Application Extension Assembly. Rev. 1.34.4.0
 */
/*
 * 02/21/2018   6.19.5.0  Vgottam   Modifications
 *                                  1. Added new project ID as PAQA for Pgh LRV.
 *                                  2. Updated subsystem name only for PAQA as "PTUP"
 *                                  3. Updated copyright year to 2018
 *                                  4. Added Project title as PAAC LRV.
 *                                  5. Added SUM document as PAQA.SUM.CHM and ReleaseNotes.PDF.
 *                                  6. Added Car ID configuration functionality.
 *                                  7. Fixed issues with Realtime datetime configuration.
 *                                  
 *                                  Dynamic Link Library Updates 
 *                                  1.  Common Application Extension Assembly. Rev. 1.34.5.0
 */
/* 05/19/2018   6.19.6.0  Vgottam   Modifications
*                                   1. Added F8 function key for Print the Plot to MS Word document.
*                                   2. Update Icon as screenshot for F2 function key.
*                                   3. Replaced WinHelp Pop-ups with HelpWindow.
*                                   Dynamic Link Library Updates 
*                                   1.  Common Application Extension Assembly. Rev. 1.34.6.0
* 06/21/2018   6.19.6.1  R.Schwar   Modifications:
*                                   1. Modified the DataGridView control properties as follows:
*                                   Made the [Car ID], [Log], [Event Name], [Date], [Time], [Day], and [Stream] columns resizeable to the user preferance.
*                                   Dynamic Link Library Updates 
*                                   1.  Events Application Extension Assembly. Rev. 1.28.4.0                                  
* 06/24/2018   6.19.6.1  Vgottam    Modifications 
*                                   1. Introduced PTU Help windows using CHM instead of HLP file extensions.
*                                   Dynamic Link Library Updates 
*                                   1.  Common Application Extension Assembly. Rev. 1.34.7.0
*/
/*
 * 07/08/2018   6.19.7.0  Vgottam    Modifications 
 *                                   1. Change the default Engineering Password from “TGX216028A” to “BTRoQEngineer”
 *                                   2. Added new project ID MAPA for MARTA and update the PTU for MAPA.
 * 
 */
/*
 * 08/02/2018   6.19.7.1  Vgottam   Modifications
 *                                  1. Updated the conditions to display as PTU name for MARTA ,BART,PAQA.
 *                                  2. Removed the display of Vista OS  from about window.
 *
 * 12/06/2018   6.19.7.2  Vgottam   Modifications
 ...
 */
/*
 * 12/21/2018   6.19.8.0  Vgottam    Modifications 
 *                                   1.  MAPA update: Enable Menu Items to set CarID and Realtime clock in Engineering and Factory level.
 *                                   2.  Updated FormViewTestResults.cs lines 723 -727 to prevent the exit self test command being sent for MAPA and PAQA projects along with R8PR.
 *                                   3.  Constant 'TimeoutMsPauseFeedback' increased 3000ms to 10000ms.
 *                                   
 * 01/22/2019   6.19.8.1  D.Smail    Modifications 
 *                                   1.  Retracted the constant 'TimeoutMsPauseFeedback' from 10000ms to 3000ms
 *                                   2.  Fixed the issue when user selects an event log from the pull-down menu list box.
