...
/*
 *  11/16/15    1.14    K.McD       References
 *                                  1.  Added support for R179.
 *
 *                                  Modifications
 *                                  1.  Added CommonConstants.ProjectIdR179 to the switch statements in the showUserMAnual() and HelpAbout() methods.
 */

/*
 *  07/202016   1.15    K.McD       References
 ...
 *  
 *                                  Modifications.
 *                                  1.  Renamed ConfigureRealTimeClock() to RealTimeClock() and moved it to the '[Tools]' region.
 */

/*
 *  09/13/2016  1.16    K.McD       References
 ...
 *  
 *                                  Modifications.
 *                                  1.  Included double quotes around each Arguments parameter associated with the ProcessStartInfo class in the ShowUserManual() method.
 */

/*
 * 10/07/2016   1.17        D.Smail   References
 ...
 *                                      
 *                                  Modifications
 *                                  1.  ShowUserManual now accepts a form instance as a parameter. In order to
 *                                      spawn a chm (help) file, the control/form must be known.
 */
/*
 * 02/11/2018   1.18        Vgotta    Modifications
 *                                  1.  Added project ID PAQA to show SUM as BART.
 */

/*
 * 02/21/2018   1.19        Vgotta    Modifications
 *                                  1.  Added new functionality to set car ID.
 */
/*
 * 07/08/2018   1.20        Vgotta    Modifications
 *                                  1.  Added project ID MAPA to show SUM as PAQA.
 *                                  
...
namespace Bombardier.PTU
{
    ...
    public class MenuInterfaceApplication : MenuInterface
    {
        ...
        /// <summary>
        /// Show the dialog box which allows the user to Configure VCU Car identication number
        /// </summary>
        public void SetCarId()
        {
            ...
            try
            {
                FormCarId formCarId = new FormCarId(MainWindow.CommunicationInterface);
                DialogResult dialogResult = MainWindow.ShowDialog(formCarId);
                if (dialogResult == DialogResult.Yes)
                {
                    MessageBox.Show(Resources.MBTCarIdSetSuccess, Resources.MBCaptionInformation, MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception)
            {
        ...
        /// <summary>
        /// Show the dialog box which allows the user to set the VCU real time clock.
        /// </summary>
        public void RealTimeClock()
        {
            ...
            try
            {
                FormRealTimeClock formConfigureRealTimeClock = new FormRealTimeClock(MainWindow.CommunicationInterface);
        ...
        /// </summary>
        /// <param name="parentForm">Reference to the parent form. Only used when chm (Help) file is spawned 
        /// and not used for launching a PDF file.</param>
        public void ShowUserManual(Form parentForm)
        {
            ...
            string fullyQualifiedFilenameSoftwareUserManual = DirectoryManager.PathPTUApplicationStartup + Resources.PathRelativeDocumentation +
                                                              CommonConstants.BindingFilename + Parameter.ProjectInformation.ProjectIdentifier + 
            ...
            if (helpDocumentFileInfo.Exists)
            {
                ...
                processStartInfo.Arguments = string.Format("\"{0}\"", fullyQualifiedFilenameSoftwareUserManual);
