 ...
 *                                      
 *  02/23/2017  1.0   D.Smail         References
 *                                      1.  Created. Used to perform periodic communication checks to the connected 
 *                                          target hardware while the user is on the main screen (form). Previously,
 *                                          no communication occurred while on the main form.
 *  
 *  06/05/2017  1.1   D.Smail         Modifications
 *                                      1.  Removed Console.Writelines() that were used for debug purposes only. No change
 *                                          to functionality
 *                                          
 */
...
namespace Bombardier.PTU
{
    ...
    /// </summary>
    class ThreadCommTarget : WorkerThread
    {
        #region --- Constants ---
        /// <summary>
        ...
        /// <summary>
        /// The thread sleep interval, in ms, between checking the state of the Pause property. Value: 500 ms.
        /// </summary>
        private int SleepMsCheckPause = 500;
            ...
            try
            {
                ...
                while (StopThread == false)
                {
                    if (Pause == false)
                    {
                        ...
                        {
                            m_Watchdog++;
                            if (m_MainForm.CommunicationInterface != null)
                            {
                                short chartMode = 1;
                                CommunicationError commError;
                                commError = m_MainForm.CommunicationInterface.WatchClockMarshall.GetChartMode(ref chartMode);
                                if (commError != CommunicationError.Success)
                                {
                                    throw new CommunicationException("TODO Need Resource String");
                                }
                            }
                            m_ReadTimeoutCountdown = ReadTimeoutCountdown;
                        }
                        catch(CommunicationException)
                        {
                            ...
                            if (m_ReadTimeoutCountdown <= 0)
                            {
                                ...
                                m_MainForm.CommunicationInterface.CloseCommunication(m_MainForm.CommunicationInterface.CommunicationSetting.Protocol);
        ...
        {
            set { m_CommunicationFault = value; }
