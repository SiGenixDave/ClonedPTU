 ...
 *                                     
 * 03/02/2017   1.24.3  D.Smail     Modifications
 *                                  1. Added ResumePollingTargetHardware() to m_MenuItemViewSystemInformation_Click() to be consistent
 *                                     with other dialog forms. Removed from FormShowSystemInformation.Close().
 */
#endregion - [1.24] -
...
#region - [1.25] -
/*
 * 02/25/2017   1.25    D.Smail     Modifications
 *                                  1.  Made changes to m_TSBOnline_Click() and m_TSBOffline_Click() to account for CTA 
 *                                      directly transitioning to Watch screen instead of Home screen. 
 *              
 ...
 *                                      
 *                                      CTA doesn't use a KeyControlPanel so a "PausePing" was inserted for CTA and
 *                                      a "ResumePing" was inserted for non-CTA projects
 * 
 */
#endregion - [1.25] -
...
#region - [1.26] -
/*
 *  09/15/2017  1.26    D.Smail     Modifications
 *                                  1.  Added a delay in m_MenuItemDiagnosticsSelfTests_Click() to support Windows XP issue for CTA (serial). When using 
...
#region - [1.27] -
/*
 *  02/21/2018  1.27    vgottam     Modifications
 *                                  1.  New event handler for new menu option "Tools/Set Car ID" to  send carId command from PTU.
 *                                  2.  Updated event handler related to menu option 'Tools/Real Time Clock' for pausing the blink and after the command to resume.
 *                                  3.  Updated resource file related to the CarID menu item size,text and tooltip information.
 * 
 */
#endregion
...
namespace Bombardier.PTU
{
    ...
    public partial class MdiPTU : Form, IMainWindow
    {
        ...
        #region - [TOOLS] -
        /// <summary>
        /// Event handler for the 'Tools/Set Car ID' menu option. Call the menu interface method that is responsible for showing the dialog box which 
        /// allows the user to configure the VCU Car Identification number.
        /// </summary>
        /// <param name="sender">Reference to the object that raised the event.</param>
        /// <param name="e">Parameter passed from the object that raised the event.</param>
        private void m_MenuItemToolsCarId_Click(object sender, EventArgs e)
        {
            PauseCommThread();
            m_MenuInterfaceApplication.SetCarId();
            ResumePollingTargetHardware();
        }
        /// <summary>
        ...
        private void m_TSBOnline_Click(object sender, EventArgs e)
        {
            ...
            if (configurationMatch == true)
            {
                ...
                if (this.Controls[CommonConstants.KeyControlPanel] == null)
                {
                    // Needed for CTA because CTA goes directly to the Watch Window upon connection
                    PauseCommThread();
                    m_MenuInterfaceWatch.ViewWatchWindow();
                }
                else
                {
                    // Start "pinging" selected target
                    ResumePollingTargetHardware();
                }
                #endregion - [Go Online] -
            }
