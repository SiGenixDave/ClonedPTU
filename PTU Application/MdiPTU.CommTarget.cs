...
/*
 *  Date        Version Author          Comments
 *  09/06/17    1.1     D.Smail         1.  Modified m_CommTimer_Tick() so that after initialization the state machines 
 *                                          transitions to the Pause state instead of the Poll state. This change was made
 *                                          to support CTA immediately transitioning to Watch screen instead of Home (like 
 *                                          all other projects).
 * 
 */
#endregion - [1.1] -
            ...
            switch (commState)
            {
                ...
                default:
                    ...
                    if (m_CommunicationInterface != null)
                    {
                        ...
                        //
                        //
                        // A check for m_Pause being false was made in case the PTU is configured to
                        // directly go to the Watch View upon successful connection to the target hardware. m_Pause will
                        // be made true if the PTU is configured to go directly to the Watch Window on connection. Since 
                        // this event always happens, the ability to support the above description is required
