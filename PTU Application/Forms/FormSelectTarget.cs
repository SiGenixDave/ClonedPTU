 ...
 *
 *                                      Modifications
 *                                      1.  Modified the GetTargets() method to include the port name adjacent to the sub-system name when displaying the list
 *                                          of targets that are connected to the system.
 */
...
/*
 *  03/11/2016  1.7     DAS             References
 *                                      1.  Bug fix - SNCR - PTU [01 Mar 2016] - Item 9. Whenever communication type is set to BOTH (serial and TCP) in the configuration
 *                                          XML file, and a serial port doesn't exist on the PC (no comm port in Device Manager), the PTU doesn't attempt to poll
 *                                          potential TCP connections.
 *
 *                                      Modifications
 *                                      1.  Modified the GetTargets() method to throw an exception if the serial communication registry key is null and the communication
 *                                          type is set to RS232. If the communication type is set to both and the registry key is null, the method will continue to check
 *                                          the specified TCPIP addresses.
 *
 */
...
/*
 *  05/18/2016  1.8     DAS             References
 *                                      1.  PTE Changes - List 5-17-2016.xlsx Item 5. Home Screen - Change 'F2-Select Target Logic' ToolStripButton Text to be
 *                                          'F2-Connect to Target.
 *
 *                                      Modifications
 *                                      1.  Changed window Text property to be 'Connect to Target'.
 *
 */
...
/*
 *  06/12/2017  1.9     DAS             Modifications
 *                                      1.  "OK" button now available as soon as the first TCP target is detected. Prior to this change all
 *                                          targets had to be scanned before the "OK" button was made available
 *                                      2.  For BART project only, the displayed URL is now a user friendly URL that lists the car Id, consist
 *                                          and truck
 *
 */
...
namespace Bombardier.PTU.Forms
{
    ...
    public partial class FormSelectTarget : FormPTUDialog
    {
        ...
        /// </summary>
        private List<TargetConfiguration_t> m_TargetConfigurationList;

        /// <summary>
        /// The list of communication settings associated with the target logic controllers that were found.
        /// </summary>
        private List<CommunicationSetting_t> m_CommunicationSettingList;
                        ...
                        foreach (string valueName in serialCommunication.GetValueNames())
                        {
                            ...
                            try
                            {
                                if (communicationInterface.ScanPort(communicationSetting, out targetConfiguration) == true)
                                {
                                    ...
                                    {
                                        targetFound = true;
                                        // Enable the OK button as soon as first target is detected
            ...
            if (Parameter.CommunicationType == Parameter.CommunicationTypeEnum.Both || Parameter.CommunicationType == Parameter.CommunicationTypeEnum.TCPIP)
            {
                ...
                foreach (string uRI in Parameter.URIList)
                {
                    ...
                    try
                    {
                        ...
                        {
                            targetConfigurationList.Add(targetConfiguration);
                            // For BART project only, the customer has requested a user friendly URL to be displayed
                            // that has the car Id, consist and truck
                            if (Parameter.ProjectInformation.ProjectIdentifier == "BART")
                            {
                                String truckId;
                                if (communicationSetting.PortIdentifier.Contains("pcux"))
                                {
                                    truckId = "X";
                                }
                                else
                                ...
                                Int32 cstIndex = communicationSetting.PortIdentifier.IndexOf("cst");
                                String consistId = communicationSetting.PortIdentifier.Substring(cstIndex + 3, 2);

                                listBoxTargetsFound.Items.Add(targetConfiguration.SubSystemName + CommonConstants.BindingMessage +
                                                              " Car: " + targetConfiguration.CarIdentifier +
                                                              " Consist: " + consistId + " Truck: " + truckId);
                            }
                            else
                            {
                                listBoxTargetsFound.Items.Add(targetConfiguration.SubSystemName + CommonConstants.BindingMessage +
