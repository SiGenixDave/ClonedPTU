 ...
 *
 *  09/17/2018  1.1     D.Axarlis   Fixed Issue with a the RTDM not finding the first VCU after opening the application by retrying the connection in case of a connection timeout.
 *
 */
    ...
    public partial class SelectVCUForm : Form
    {
        ...
        private Boolean TestFTPConnection(string ftpHost)
        {
            ...
            try
            {
                ...
                // Allow 500 msecs for an FTP response. If the timeout is triggered a single attempt for reconnection will be made. 
                // Typically, if the connection never existed, the code immediatelyfalls thru to catch()
                requestDir.Timeout = 500; //Changed To 500 from 5000 when allowing for a retry on a timeout.
                requestDir.GetResponse();
                ftpConnectionState = true;
            }
            catch (WebException e) // Catch Timeout Exception and attempt another connection
            {
                // Check for a Timeout Exception
                if (e.Status == WebExceptionStatus.Timeout)
                {
                    //MessageBox.Show(ftpHost + e.Message);//Added to debug message.
                    try
                    {
                        requestDir = (FtpWebRequest)FtpWebRequest.Create(ftpHost);
                        requestDir.Method = WebRequestMethods.Ftp.ListDirectoryDetails;
        ...
        private void SelectVCUForm_Load(object sender, EventArgs e)
        {
