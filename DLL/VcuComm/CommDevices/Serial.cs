 ...
 *
 *                                  Modifications
 *                                  1.  Modified the ReceiveMessage() method. If a read is performed and zero chars are read, then this indicates a buffer
 *                                      overflow would have occurred. Therefore, handle this condition by returning an error code. NOTE: If the cable is removed, a
 *                                      timeout would occur.
 *
...
namespace VcuComm
{
    ...
    public class Serial : ICommDevice
    {
        ...
        /// <summary>
        /// Send a message to the embedded PTU target. For every byte in the message, the software waits for n
        /// echo of the transmitted byte from the target. After every byte is transmitted and received, the software
        /// then verifies that the transmitted message is identical to the received message.
        /// </summary>
        /// <param name="txMessage">the message to be sent to the target</param>
        /// <returns>less than 0 if any failure occurs; 0 if successful</returns>
        public Int32 SendMessageToTarget(byte[] txMessage)
        {
            ...
            foreach (byte b in txMessage)
            {
                // Store the byte to be transmitted in an array... required for serial port API
                byte[] txArray = new byte[] { b };

                // Send the entire message to the serial port
                try
                {
                    m_SerialPort.Write(txArray, 0, txArray.Length);
                }
                catch (Exception e)
                {
                ...
                // wait for the target logic to echo back the exact byte just sent by the application
                // (NOTE: this is different than TCP which doesn't expect an echo of the transmitted message)
                Int32 bytesRead = 0;
                while (bytesRead != txArray.Length)
                {
                    bytesRead = ReceiveMessage(rxMessage, totalBytesRead);
                    // bytesRead will be less than 0 if ReceiveMessage() encounters an error
                    if (bytesRead < 0)
                    {
                        return bytesRead;
                    }

                    // Should never read back more than 1 byte (the target echo)
                    if (bytesRead > txArray.Length)
                    {
                        return -1;
                    }
            ...
            // This compares the contents of the 2 arrays (TX and RX message)
        ...
        /// <summary>
        /// Sends the Start Of Message (SOM) to the target and waits for the target's SOM
        /// </summary>
        /// <returns>less than 0 if an error is detected; 0 otherwise</returns>
