...
/*
 *  07/20/2016  2.5.2  K.McD        References
 ...
 *                                  2.  Changed the event variable user control width constants to: 190, 180 and 100 for the description, value and units fields
 *                                      respectively. - Ref.: 4.
 *                                  3.  Added a call to base.FormPTU_Resize() in the FormPTU_Resize() event handler. - Ref.: 3.
 *                                  4.  Added the override function to the F1_Click() event handler. This toggles the Enumeration control flag and updates the
 *                                      event variable list associated with the selected event. - Ref.: 1.
 */
...
namespace Event.Forms
{
    ...
    public partial class FormViewEventLog : FormPTU, IPollTarget, ICommunicationInterface<ICommunicationEvent>
    {
        ...
        /// </summary>
        protected const int WidthEventControlVariableNameField = 190;

        /// <summary>
        /// The width, in pixels, of the value field of the event variable user control. Value: 180.
        /// </summary>
        protected const int WidthEventControlValueField = 180;
        ...
        /// <param name="sender">Reference to the object that raised the event.</param>
        /// <param name="e">Parameter passed from the object that raised the event.</param>
        protected override void F1_Click(object sender, EventArgs e)
        {
            // Skip, if the Dispose() method has been called.
            if (IsDisposed)
            {
            ...
            base.F1_Click(sender, e);

            Cursor = Cursors.WaitCursor;

            // Update the event variable list values for the selected event.
        ...
        protected void ConfigureControls(Panel panel, VariableControlSize_t eventControlSize, short startIndex, List<EventVariable> eventVariableList)
        {
            ...
            for (short index = startIndex; index < eventVariableList.Count; index++)
            {
                ...
                eventVariableControl.ForeColorValueFieldZero = Color.Black;
                eventVariableControl.ForeColorValueFieldNonZero = Color.Black;
        ...
        private bool Save()
        {
            ...
            foreach (EventRecord record in EventRecordList)
            {
                ...
                if (_ev != null)
                {
                    truckType = Lookup.EventVariableTable.GetEnumeratorText(_ev.Identifier, (uint)_ev.ValueFromTarget);
                    break;
                }
            }
