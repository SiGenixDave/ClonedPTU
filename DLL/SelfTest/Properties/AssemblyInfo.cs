 ...
 *                                      
 *                                  Modifications
 *                                  1.  \Communication\CommunicationSelfTest.cs Rev 1.3.
 *                                  2.  Removed \Communication\VcuCommunication32SelfTest.cs, \Communication\VcuCommunication64SelfTest.cs.
 */
#endregion - [1.8] -

#region - [1.9] -
/*
 * 04/07/2016   1.9     K.McD       References
 ...
 *                                  
 *                                  Modifications
 *                                  1.  Updated Resources.resx. - Ref.: 1.
 *                                          Added:      EMSelfTestEnterExceptionThrown, EMSelfTestEnterReasonNotGiven, EMSelfTestEnterReasonNotGiven and
 *                                                      EMSelfTestEnterNoResponse.
 *                                          Deleted:    EMResultFailed and EMStartSelfestTaskFailed.
 *                                          
 *                                  2.  Modified FormViewTestResults.cs. Rev. 1.12. - Ref.: 1.
 */

/*
 *  04/21/2016  1.9.1   K.McD       References
 *                                  1.  SELFTESTERRMESS table of TOPC3607.E1. 
 *                                      
 *                                  Modifications
 *                                  1.  Resources.resx. Modified the text of the Self Test entry error messages to match the format used in the SELFTESTERRMESS table
 *                                      of the '.E1' data dictionary file.
 */

/*
 *  07/11/2016  1.9.2   K.McD       References
 ...
 *                                      
 *                                  Modifications
 *                                  1.  FrmViewTestResults.cs. Rev. 1.13. - Ref.: 1.
 *                                  2.  FormTestListDefine.cs. Rev. 1.4. - Ref.: 2.
 *                                  3.  FormTestListDefine.Designer.cs. Rev. 1.4. - Ref.: 2.
 *                                  4.  Resources.resx. Added the LegendItems string resource.
 */

/*
 *  07/20/2016  1.9.3   K.McD       References
 *                                  1.  PTE Changes - List 5-17-2016.xlsx Item 62. On all screens, change the green text to black.
 *                                  
 *                                  Modifications
 *                                  1.  FormViewTestResults.cs. Rev. 1.13.1. 
 */

/*
 *  08/01/2016  1.9.4   K.McD       References
 ...
 *                                      
 *                                  2.  PTE Changes - List 5-17-2016.xlsx Item 59. On the 'View/Watch Window' and 'Diagnostics/Self-Tests' screens, change the name
 ...
 *                                  
 *                                  Modifications
 *                                  1.  FormTestListDefine.cs. Rev. 1.5. - Ref.: 1.
 *                                  2.  FormTestListDefine.Designer.cs. Rev. 1.5. - Ref.: 2.
 *                                  3.  Resources.resx. - Ref.: 2.
 *                                          1.  FunctionKeyTextEdit changed from 'Modify' to 'Config'.
 *                                          2.  FunctionKeyToolTipEdit changed from [Modify Test List] to [Configure Test List]
 */

/*
 *  08/01/2016  1.9.5   D.S         References
 *                                  1.  Bug - Self test results screen locked up when running interactive test.
 *                              
 *                                  Modifications
 *                                  1.  FormViewTestResults.cs. Rev. 1.14. - Ref.: 1.
 */
/*
 *  02/21/2017  1.9.6  Vgott        References
...
#region - [1.10] -
/*
 *  02/XX/2017     1.10.1.0   D.Smail  Modifications
 ...
 *                                      3.  Now that self test watchdog message response from target hardware includes a byte which indicates
 ...
 *                                      4.  Fix issue with "non Self test watchdog" code that gracefully exists self test when a loss of communication
 ...
 *                                      5.  When communication is lost to the target hardware, disable all controls
 ...
 *                                      6.  When communication is lost to the target hardware or the target hardware exits
 *                                          self test on its own, disable the self test tab control so that the user can't
 *                                          select any test. 
 *                                      7.  When communication is lost to the target hardware, inform the Main Window of 
 *                                          this event by making the Main Window's Communication Interface = null.
 *                                      8.  Upon exiting the Self Test Form, inform the Main Window to resume polling the 
 *                                          target hardware.
 *                                      9.  Added file ThreadCommWatchdog.cs file to support background "pinging" during self test to detect loss of comm
 *                                     10.  Added CommunicationWatchdog method to the interface.
 *                                     11.  Removed all references to PTUDLL32 in method descriptions.
 *                                     12.  Added a reference parameter to CommunicationWatchdog() method that is updated to indicate
 *                                          whether or not the target hardware is in self test.
 *                                  
 * 
 */
/*
 *  06/23/2017     1.10.2.0   Vgott   References
 *                                    1.  Update the tooltip text for F5-Config.                 
 *                                  
 *                                    Modifications
 *                                    1.  Update the tooltip text as "Open Test List Configuration window" instead "Configure Test List".
 * 
 */
/*
 *  09/12/2017     1.10.3.0   Vgott   Modifications
 *                                    1.  Disabled Menus of main window,Menu: Diagnostics and pull down menu item: View->WatchWindow.
 *                                  
 *  09/15/2017     1.10.3.1   D.Smail Modifications
 ...
 *                                        
 *  12/21/2018     1.10.3.2  Vgottam  Modifications
 *                                    1.  Updated FormViewTestResults.cs lines 723 -727 to prevent the exit self test command being sent for MAPA and PAQA projects along with R8PR.
