 ...
 *                                          target hardware.
 *                                          
 *  03/02/17    1.15.6  D.Smail         Modifications
 *                                      1.  When the system aborts self test, the PTE now informs the user in the message 
 *                                          status box, disables all buttons and requires the user to click "Home". However, 
 *                                          after going back to the main form and clicking on any one of the buttons (Event 
 *                                          Log, Data monitor, etc) after a self test abort, this would cause a MessageBox 
 *                                          error to popup. After debugging, I discovered an extra line of code that 
 *                                          shouldn't have been included in the PTE software (Set the MainWindow.CommunicationInterface 
 *                                          to null) when it detected that the VCU has aborted self test. Removed that 
 *                                          line of code because Communication has not been lost.
 * 
 *  09/12/2017  1.15.7   Vgott           Modifications
 *                                       1.  Disabled Menu items KeyMenuItemDiagnostics and KeyMenuItemView. 
 *  
 *  09/15/2017  1.15.8   D.Smail         Modifications
 ...
 *                                          
 *  09/15/2017  1.15.9   Dillon         Modifications
 *                                      1. Updated 725 -727 lines to prevent the exit self test command being sent for MAPA and PAQA projects along with R8PR.
 */
#endregion - [1.15] -
