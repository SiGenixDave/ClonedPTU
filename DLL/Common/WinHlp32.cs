 ...
 *
 *                                      
 *  05/07/18    1.5     Vgottam         1. Updated the ShowPopup function with ShowHelpWindow  due to the no scrolling bar for the popups.
 *  
 *  06/24/18    1.5     Vgottam         1. Introduced PTU Help windows using CHM instead of HLP file extensions to support Operation systems greater than Windows 7.
 */
...
namespace Common
{
    ...
    public class WinHlp32
    {
        ...
        /// <summary>
        /// A flag to indicate whether the Windows Operating system is heigher than Windows 7 OS. True, if the OS is greater than Windows 7; otherwise, false.
        /// </summary>
        private static bool m_isVersonGreaterThanWindow7;
        ...
        public static void Initialize(string helpFilename)
        {
            ...
            if  (System.Environment.OSVersion.Version.Major > 6)
                m_isVersonGreaterThanWindow7 = true;
            else if (System.Environment.OSVersion.Version.Major == 6)
            {
                if (Environment.OSVersion.Version.Minor > 1)
                    m_isVersonGreaterThanWindow7 = true;
                else
                    m_isVersonGreaterThanWindow7 = false;
            }
            else
                m_isVersonGreaterThanWindow7 = false;
                     
            // Check that the specified help file exists and has the correct extension.
            helpFilename = (!m_isVersonGreaterThanWindow7) ? helpFilename : helpFilename.ToLower().Replace(CommonConstants.ExtensionHelpFile, CommonConstants.ExtensionAltHelpFile);
            FileInfo fileInfo = new FileInfo(helpFilename);
            if (fileInfo.Exists == false)
            {
                throw new ArgumentException(Resources.EMFileNotFound, "helpFilename");
            }
            else if (fileInfo.Extension != CommonConstants.ExtensionHelpFile && fileInfo.Extension !=CommonConstants.ExtensionAltHelpFile)
            {
        ...
        public static void Close(int winHandle)
        {
            ...
            if (IsInitialized == true && !m_isVersonGreaterThanWindow7)
            {
        ...
        public static void ShowHelpWindow(int winHandle, int helpIndex, int hWndInsertAfter)
        {
            ...
            if (IsInitialized == true)
            {
                ...
                if (m_isVersonGreaterThanWindow7)
                {
                    System.Windows.Forms.Control cntl = System.Windows.Forms.Control.FromHandle((IntPtr)winHandle);
                    System.Windows.Forms.Help.ShowHelp(cntl, m_FullQualifiedHelpFilename, System.Windows.Forms.HelpNavigator.Topic, helpIndex.ToString() + ".html");
                }
                else
                {
                ...
                SetWindowPos(hwnd, HWND_TOP, x, y, 450, 350, SWP_NOZORDER);
        ...
        public static void ShowPopup(int winHandle, int helpIndex)
        {
            ...
            if (IsInitialized == true)
            {
                ...
                int x, y;
                int hwnd;
                //inactive the show popup option
                if (usePopup)
                {
                    success = WinHelp(winHandle, m_FullQualifiedHelpFilename, HelpContextPopup, helpIndex);
                    if (success != true)
                    {
                        // No need to throw an exception as the help system will show a window explaining why the call failed.
                    }
                ...
                else
                {
                    ...
                    {
                        success = WinHelp(winHandle, m_FullQualifiedHelpFilename, HelpContext, helpIndex);
                        if (success != true)
                        {
                            // No need to throw an exception as the help system will show a window explaining why the call failed.
                        }
                    ...
                    // Set the window Z order to be defined by the hWndInsertAfter parameter.
                    SetWindowPos(hwnd, WinHlp32.HWND_TOP, 0, 0, 0, 0, SWP_SHOWWINDOW | SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOMOVE);
