 ...
 *  07/14/2018   1.20   Vgott          Modifications
 *                                      1. Added F8 function key for Print the Plot to MS Word document or Directly Print with preview option in case of no MS Word installed.
 *                                      2. Updated F8 function key to handle the missing "m_LabelUnits" control.
 */
...
namespace Common.Forms
{
    ...
    public partial class FormDataStreamPlot : FormPTU, IWatchFile
    {
        ...
        /// <summary>
        /// Reference to the list that holds bitmap data of the Data stream Plots on the FormDataStreamPlots. This is called using function key F8.
        /// </summary>
        private List<Bitmap> m_listPlotBitmaps = new List<Bitmap>();
        /// <summary>
        /// Reference to the list that holds plot description of the Data stream Plots on the FormDataStreamPlots. This is called using function key F8.
        /// </summary>
        private List<string> m_listPlotsDescription = new List<string>();
        /// <summary>
        /// Reference to the list that holds plot units of the Data stream Plots on the FormDataStreamPlots. This is called using function key F8.
        /// </summary>
        private List<string> m_listPlotsUnits = new List<string>();
        /// <summary>
        /// Reference to counter of the plots in the FromDataStreamPlot window, it used save plots in multiple pages and gets incremented till it reaches to no.of plots in the Form.
        /// It is called in function Key F8 and Print document event handler.
        /// </summary>
        ...
        public FormDataStreamPlot(WatchFile_t watchFile)
        {
            ...
            DisplayFunctionKey(F8, Resources.FunctionKeyTextPrint, Resources.FunctionKeyToolTipPrint, Resources.Print);
        ...
        protected override void F8_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               m_listPlotBitmaps = new List<Bitmap>();
               m_listPlotsDescription = new List<string>();
               m_listPlotsUnits = new List<string>();
                foreach (Control control in this.m_TabControl.TabPages[0].Controls)
                {
                    foreach (Control subControl in control.Controls)
                    {
                        IPlotterWatch plotter = subControl as IPlotterWatch;
                        if (plotter != null)
                        {
                            if (plotter.Removed == true)
                                continue;

                        }
                        Control sub1,sub2;
                        if (subControl.Controls.Find("m_LabelDescription", true).Length <= 0)                            
                            continue;
                        else
                        sub1 = subControl.Controls.Find("m_LabelDescription", true)[0];

                        if (subControl.Controls.Find("m_LabelUnits", true).Length <= 0)
                            sub2 = new Control(" ");
                        else
                            sub2 = subControl.Controls.Find("m_LabelUnits", true)[0];
                         
                        
                        if (sub2 == null || sub1 == null)
                            continue;
                        m_listPlotsDescription.Add(sub1.Text);
                        m_listPlotsUnits.Add(sub2.Text);
                        Bitmap bmp = new Bitmap(subControl.Bounds.Width, subControl.Bounds.Height);
                        subControl.DrawToBitmap(bmp, new System.Drawing.Rectangle(0, 0, subControl.Width, subControl.Height));
                        m_listPlotBitmaps.Add(bmp);


                    }

                }
                bool is_WordEnable = false;                
                DialogResult dr = MessageBox.Show(Resources.MBTPrintOption, Resources.MBTPrintOptionCaption, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                try
                {
                ...
                {
                    MessageBox.Show(Resources.MBTPrintNoMSWord, Resources.MBTPrintNoMSWordCaption, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    is_WordEnable = false;
                }
                if (is_WordEnable)
                {
                    DateTime createdTime = DateTime.Now;
                    string extension = CommonConstants.Period + ScreenCaptureType.Plot.ToString() + CommonConstants.ExtensionDoc;
                    string defaultFilename = General.DeriveName(FileHeader.HeaderCurrent.TargetConfiguration.CarIdentifier, createdTime, extension, string.Empty, string.Empty);
                    string fullFilename = General.FileDialogSaveDocFile(defaultFilename, InitialDirectory.FaultLogsWrite);
                    if (fullFilename != string.Empty)
                    {
                        FileInfo fileInfo = new FileInfo(fullFilename);
                        // Update the initial directory with the path of the selected file.
                        InitialDirectory.FaultLogsWrite = fileInfo.Directory.ToString();
                        ...
                        oWord.Visible = false;
                        Object oBookmark = this.Text;
                        Range oRng = oDoc.Sections.First.Range;

                        for (int i = 0; i < m_listPlotBitmaps.Count; i++)
                        {
                            Paragraph para = oDoc.Content.Paragraphs.Add(Type.Missing);
                            Table tab = para.Range.Tables.Add(para.Range, 2, 2, Type.Missing, WdAutoFitBehavior.wdAutoFitContent);
                            tab.Cell(1, 1).Range.Text = m_listPlotsDescription[i];
                            tab.Cell(1, 1).Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphLeft;
                            tab.Cell(1, 2).Range.Text = m_listPlotsUnits[i];
                            tab.Cell(1, 2).Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphRight;
                            para.Range.InsertParagraphAfter();
                            tab.Cell(2, 2).Merge(tab.Cell(2, 1));
                            Clipboard.SetDataObject(m_listPlotBitmaps[i]);
                            tab.Cell(2, 1).Range.Paste();
                        }
                        ...
                        foreach (Section section in oDoc.Sections)
                        {
                            ...
                            section.Range.Font.Size = 10.0F;
                            Table tab = section.Range.Tables.Add(headerRange, 1, 2, Type.Missing, WdAutoFitBehavior.wdAutoFitFixed);
                            tab.Cell(1, 1).Range.Text = this.MainWindow.ProjectIdentifierPassedAsParameter;
                            tab.Cell(1, 1).Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphLeft;
                            tab.Cell(1, 1).Width = 40;
                            tab.Cell(1, 2).Range.Text = this.Text;
                            tab.Cell(1, 2).Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphRight;
                            tab.Cell(1, 2).Width = 410;
                        }

                        oDoc.SaveAs(fullFilename, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing);
                        oWord.Quit();
                ...
                else
                {
                    ...
                    m_printPreviewDialogPTU.Document = pd;
                    if (m_printPreviewDialogPTU.ShowDialog() == DialogResult.OK)                    
                        pd.Print();
        ...
        {
            System.Drawing.Font printBodyTextFont = new System.Drawing.Font("Calibri", 10);
            System.Drawing.Font printHeaderFont = new System.Drawing.Font("Calibri", 11);
            float yPos = 0;

            float leftMargin = e.MarginBounds.Left;
            float topMargin = e.MarginBounds.Top;
            int bodyHeight = e.MarginBounds.Height;
            //Header print
            e.Graphics.DrawString(this.MainWindow.ProjectIdentifierPassedAsParameter, printHeaderFont, Brushes.Black,
                   leftMargin, e.MarginBounds.Top / 2, new StringFormat());
            SizeF sizeofLeftHeader = e.Graphics.MeasureString(this.MainWindow.ProjectIdentifierPassedAsParameter, printHeaderFont);
            SizeF sizeofRightHeader = e.Graphics.MeasureString(this.Text, printHeaderFont);
            if (sizeofRightHeader.Width > (e.MarginBounds.Width - (leftMargin + sizeofLeftHeader.Width)))
            {
                float percharwidth = sizeofRightHeader.Width / this.Text.Length;
                float diffWidth = sizeofRightHeader.Width - (e.MarginBounds.Width - (leftMargin + sizeofLeftHeader.Width));
                int textSpliLen = Convert.ToInt32(diffWidth / percharwidth);
                textSpliLen = textSpliLen > this.Text.Length ? (this.Text.Length / 10) : textSpliLen;
                e.Graphics.DrawString(this.Text.Substring(0, this.Text.Length - textSpliLen - 1), printHeaderFont, Brushes.Black,
                         e.MarginBounds.Right, (e.MarginBounds.Top / 2) - sizeofRightHeader.Height / 2, new StringFormat(StringFormatFlags.DirectionRightToLeft));

                e.Graphics.DrawString(this.Text.Substring(this.Text.Length - textSpliLen - 1), printHeaderFont, Brushes.Black,
                         e.MarginBounds.Right - percharwidth * textSpliLen, (e.MarginBounds.Top / 2) + sizeofRightHeader.Height / 2, new StringFormat());

            }
            else
            {
                e.Graphics.DrawString(this.Text, printHeaderFont, Brushes.Black,
                        e.MarginBounds.Right, e.MarginBounds.Top / 2, new StringFormat(StringFormatFlags.DirectionRightToLeft));
            }

            yPos = topMargin;
            if (m_listPlotsDescription.Count > 0)
            {
                while (m_count < m_listPlotsDescription.Count)
                {
                    if (m_count < m_listPlotsDescription.Count && (yPos + 10 >= e.MarginBounds.Height))
                    {
                    ...
                    yPos += printBodyTextFont.GetHeight(e.Graphics);
                    e.Graphics.DrawString(m_listPlotsDescription[m_count], printBodyTextFont, Brushes.Black,
                   leftMargin, yPos, new StringFormat());

                    e.Graphics.DrawString(m_listPlotsUnits[m_count], printBodyTextFont, Brushes.Black,
                  e.MarginBounds.Right, yPos, new StringFormat(StringFormatFlags.DirectionRightToLeft));
                    yPos += printBodyTextFont.GetHeight(e.Graphics);
                    Stream imageStream = new MemoryStream();
                    m_listPlotBitmaps[m_count].Save(imageStream, ImageFormat.Jpeg);
                    System.Drawing.Rectangle destRect = new System.Drawing.Rectangle(Convert.ToInt32(leftMargin), Convert.ToInt32(yPos), e.MarginBounds.Width, m_listPlotBitmaps[m_count].Size.Height);

                    e.Graphics.DrawImage(Image.FromStream(imageStream), destRect, new System.Drawing.Rectangle(0, 0, m_listPlotBitmaps[m_count].Width, m_listPlotBitmaps[m_count].Height), GraphicsUnit.Pixel);
                    yPos += m_listPlotBitmaps[m_count].Size.Height;
                    m_count++;
                }

            }

            if (m_count >= m_listPlotsDescription.Count)
            {
                e.HasMorePages = false;
                m_count = 0;
            }
        }
        #endregion - [Print document Event Handler] -
        #region - [UserControl Event Handlers] -
        /// <summary>
