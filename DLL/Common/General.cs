 ...
 */
/*
 * 06/23/2017  1.21.2       Vgott   References
 ...
 *                                  
 *                                  Modifications
 *                                  1.  Modified the DeriveName() method to include "Truck Type". and also updated the places where this method is used.
 *
 * 02/11/2018  1.21.3       Vgott   Modifications
 *                                  1.  Updated the subsystem name as "PTUP" instead of "PTEP" for PAQA project.
 *                                  
 * 05/19/2018  1.21.4       Vgott   Modifications
 *                                  1.  Added new file dialog to save Print Datastream Polts files in MS Word format.
 *                                  
 * 07/08/2018  1.21.5       Vgott   Modifications
 *                                  1.  Updated the code for new project ID: MAPA.  
 *                                  
...
namespace Common
{
    ...
    public class General
    {
        ...
        /// <summary>
        /// Set-up the <c>SaveFileDialog</c> parameters associated with saving a Data streams Print document.
        /// </summary>
        /// <param name="defaultFilename">The default filename.</param>
        /// <param name="initialDirectory">The path of the initial directory that is to be displayed.</param>
        /// <returns>The fully qualified filename of the specified file or 'string.Empty' if no file is specified.</returns>
        public static string FileDialogSaveDocFile(string defaultFilename, string initialDirectory)
        {
            SaveFileDialog saveFileDialog = new SaveFileDialog();
            saveFileDialog.InitialDirectory = initialDirectory;
            saveFileDialog.DefaultExt = CommonConstants.ExtensionDoc.Remove(0, 1);
            saveFileDialog.Title = Resources.FileDialogSaveTitleDocFile;
            saveFileDialog.Filter = Resources.FileDialogSaveFilterDoc + "|" + "*" + CommonConstants.ExtensionDoc;

            saveFileDialog.FileName = defaultFilename;

            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {                
               saveFileDialog.FileName = Path.ChangeExtension(saveFileDialog.FileName, CommonConstants.ExtensionDoc); 
               return saveFileDialog.FileName;
            }
            else
            {               
        ...
        public static string GetFullyQualifiedFaultLogFilename(EventRecord eventRecord, out string defaultFilename)
        {
            ...
            if (eventRecord.StreamSaved == true)
            {
                ...
                defaultFilename = General.DeriveName(eventRecord.CarIdentifier, eventRecord.DateTime, CommonConstants.ExtensionFaultLog,
                                                     eventRecord.Description, string.Empty);
        ...
        /// <param name="truckType">The description that is to be added to the file.</param>
        /// <returns>The derived default filename of the file.</returns>
        public static string DeriveName(string carIdentifier, DateTime dateTime, string extension, string description,string truckType)
        {
            ...
            switch (Parameter.ProjectInformation.ProjectIdentifier)
            {
                ...
                case CommonConstants.ProjectIdAPM:
                    ...
                    break;
                default:
                    // Generate the name based upon the specified dateTime - '<project-ID> [<car-identifier><TruckType>] [<description>] YYMMDD-HHmmss.<extension>'.
                    
                    if (description.Equals(string.Empty))
                    {
                        format = FileHeader.HeaderCurrent.ProjectInformation.ProjectIdentifier + CommonConstants.Space +
                                 "[" + carIdentifier + truckType + "]" + CommonConstants.Space +
